...
<?php
	if(($this->session->userdata('level') == 'admin') || ($this->session->userdata('level') == 'user') AND ($this->session->userdata('status') == 'aktif')){
?>
<a href='<?php echo base_url(); ?>first/add_musik'><div class='b'>Tambah</div></a>
<hr>
...
<table border='0' align='center'>
	...
	<tr <?php if($i % 2 == 1){echo "bgcolor='#e2e5ef'";}else{echo "bgcolor='#bfddff'";}?>>
		...
		<td><a href='<?php echo base_url(); ?>first/download/<?php echo $row->file; ?>'>Download</a> | <a href='<?php echo base_url();?>first/hapus_musik/<?php echo $row->id_album; ?>/<?php echo $row->file; ?>'>Hapus</a></tr>
	</tr>
	<?php 
	$i++;
 endforeach; ?>
